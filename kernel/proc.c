...
userinit(void)
{
 ...
 //p->context.sp = PGSIZE;  // user stack pointer
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        printf("The process to run is: %s\n", p->name);
...
void uart_console_loop() {
    for (;;) {
        uartintr();
        yield(); // Yield control to allow other processes to run.
    }
}
